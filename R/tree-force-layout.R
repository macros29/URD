...
treeForceDirectedLayout <- function(object, num.nn=NULL, dim = 2, method=c("fr", "drl", "kk"), cells.to.do=NULL, cell.minimum.walks=1, cut.outlier.cells=NULL, cut.outlier.edges=NULL, max.pseudotime.diff=NULL, cut.unconnected.segments=2, min.final.neighbors=2, remove.duplicate.cells=T, tips=object@tree$tips, coords="auto", start.temp=NULL, n.iter=NULL, density.neighbors=10, plot.outlier.cuts=F, verbose=F) {
  ...
  if (verbose) print(paste0(Sys.time(), ": Starting with parameters: ", "method: ", method, ", num.nn: ", num.nn, "NN, ", "dim: ", dim, "D, ", length(cells.to.do), " cells"))
  ...
  walk.data <- as.data.frame(walk.data)
