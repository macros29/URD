...
#' keywords internal
#' @exports
floodPseudotimeCalc <- function(start.cells, minimum.cells.flooded=2, tm.flood=NULL, verbose.freq=0) {
  # Build a normalized transition matrix for flooding, unless it's passed to the function.
  if (is.null(tm.flood)) {
    # if (verbose) print(paste0(Sys.time(), ": tm.flood not specified, so calculating normalized transition matrix."))
    # tm.flood <- floodBuildTM(object
    stop(paste0(Sys.time(), ": tm.flood not specified. Please calculate tm.flood using floodBuildTM function first."))
  }
...
#' @param n_thread (Numeric) number of threads '
...
floodPseudotime <- function(root.cells, n=20, n_thread=NULL, minimum.cells.flooded=2, tm.flood=NULL, verbose=F, verbose.freq=10) {
  ...
  floods <- as.data.frame(future.apply::future_lapply(1:n, function(x) {
    if (verbose) print(paste("Starting flood number", x))
    floodPseudotimeCalc(start.cells = root.cells, minimum.cells.flooded = minimum.cells.flooded, tm.flood = tm.flood, verbose.freq=verbose.freq)
  }))
  colnames(floods) <- 1:n
  return(floods)
}
